...
VIDEO_PROCESSED = Counter(
    "videos_processed_total", "Total Videos Processed", ["tier", "video_type", "status"]
)
...
AI_COST = Counter(
    "ai_processing_cost_total", "Total AI Processing Cost in USD", ["service", "tier"]
)

USER_SIGNUP = Counter("user_signups_total", "Total User Signups", ["tier"])

TIER_UPGRADE = Counter(
    "tier_upgrades_total", "Total Tier Upgrades", ["from_tier", "to_tier"]
)

# System Metrics
ACTIVE_USERS = Gauge("active_users_current", "Current Active Users")

QUEUE_LENGTH = Gauge("processing_queue_length", "Current Processing Queue Length")
...
DATABASE_CONNECTIONS = Gauge(
    "database_connections_current", "Current Database Connections"
)
...
def record_video_processing(tier, video_type, duration, status="completed"):
    ...
    VIDEO_PROCESSED.labels(tier=tier, video_type=video_type, status=status).inc()

    VIDEO_PROCESSING_TIME.labels(tier=tier, video_type=video_type).observe(duration)


def record_ai_cost(service, tier, cost):
    """Record AI processing cost."""
    AI_COST.labels(service=service, tier=tier).inc(cost)
...
def record_tier_upgrade(from_tier, to_tier):
    """Record tier upgrade."""
    TIER_UPGRADE.labels(from_tier=from_tier, to_tier=to_tier).inc()
