        ...
        try:
            ...
            try:
                ...
                if not user or not user.is_active:
                    logger.warning(
                        f"Invalid user attempting WebSocket connection: {user_id}"
                    )
                    disconnect()
                    return False
...
# In websocket.py, update the send functions:
def send_video_update(
    ...
    if _socketio is None:
        logger.warning("WebSocket not initialized, skipping update")
        return
...
def send_tier_upgraded(
    ...
    try:
        ...
        logger.info(
            f"Tier upgrade notification sent: {user_id} ({old_tier} -> {new_tier})"
        )
...
def broadcast_maintenance_mode(
    ...
    try:
        ...
        logger.info(
            f"Maintenance mode broadcast: {'enabled' if enabled else 'disabled'}"
        )
...
def get_socketio():
    """Get the socketio instance (returns None if not set)."""
    return _socketio
