...
def upload_video():
    ...
    try:
        ...
        response_data["original_audio_quality"] = getattr(
            video, "original_audio_quality", "unknown"
        )
        response_data["original_aspect_ratio"] = getattr(
            video, "original_aspect_ratio", "unknown"
        )
        response_data["original_resolution"] = getattr(
            video, "original_resolution", "unknown"
        )
...
def apply_styles_to_video(video_id):
    ...
    try:
        ...
        #  Update video status to 'processing' BEFORE starting task
        from providers.firebase_provider import FirebaseProvider
        ...
        # Also update the video object in memory
        video.status = "processing"
        video.current_step = "applying_style"
        video.progress = 10
...
@router.route("/task/<task_id>/status", methods=["GET"])
@jwt_required()
def get_task_status(task_id):
    """Get task status from Redis (no database needed)."""
    ...
    try:
        ...
        if status:
            ...
            # If not in Redis, check Celery directly
                    ...
                    jsonify(
                        {
                            ...
                            "step": info.get("current_step", "processing"),
...
@router.route("/task/<task_id>/result", methods=["GET"])
@jwt_required()
def get_task_result(task_id):
    ...
    try:
        ...
        if task.state == "SUCCESS":
            ...
            return jsonify({"success": False, "error": str(task.info)}), 200
        else:
            return (
                jsonify(
                    {
...
def get_video_status(video_id):
    ...
    try:
        ...
        return jsonify({"error": str(e), "video_id": video_id, "status": "error"}), 500
...
def check_thumbnails_status(video_id):
    ...
    try:
        ...
        status = {"ai_thumbnails": [], "extracted_thumbnails": [], "ready": False}
            ...
            status["ai_thumbnails"].append(
                {
                    ...
                    "exists": exists,
                    "url": (
                        f"/api/v1/videos/thumbnails/{thumb_path}"
                        if thumb_path
                        else None
                    ),
                }
        ...
        status["ready"] = (
            all(t["exists"] for t in status["ai_thumbnails"])
            if status["ai_thumbnails"]
            else True
        )
