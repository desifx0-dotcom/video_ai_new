...
class FFmpegProvider:
    ...
    def _test_ffmpeg(self):
        ...
        try:
            ...
            raise ProcessingError("FFmpeg not found or not working. Please install FFmpeg.")
    ...
    def get_video_metadata(self, video_path: str) -> Dict[str, Any]:
        ...
        try:
            cmd = [
                self.ffprobe_path,
                '-v', 'error',
                '-show_entries', 'stream=codec_type,codec_name,width,height,pix_fmt,duration,r_frame_rate,bit_rate',
            ...
            metadata = {
                ...
                'video': {
                    ...
                    'fps': fps,
                    'bitrate': int(video_stream.get('bit_rate', 0)) if video_stream else None
                },
                'audio': {
                    'codec': audio_stream.get('codec_name', '') if audio_stream else None,
                    'bitrate': int(audio_stream.get('bit_rate', 0)) if audio_stream else None
                }
            }
    ...
    def extract_frame(
        ...
        try:
            cmd = [
                self.ffmpeg_path,
                '-ss', str(timestamp),  # Seek to position
                '-i', video_path,
    ...
    def create_thumbnail_grid(
        ...
        try:
            ...
            for i, timestamp in enumerate(timestamps):
                ...
                if self.extract_frame(video_path, timestamp, temp_dir, f'frame_{i:03d}'):
