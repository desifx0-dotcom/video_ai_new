...
class TierLimitExceeded(VideoAIException):
    ...
    def __init__(
        ...
        self.limit = limit
        self.upgrade_url = upgrade_url or "/pricing"

        if message:
            self.message = message
        else:
            self.message = f"{limit_type} limit exceeded: {current}/{limit}"

        super().__init__(self.message, "TIER_LIMIT_EXCEEDED", 403)
...
class InsufficientCreditsError(VideoAIException):
    ...
    def __init__(
        ...
        Args:
            current: Current credits available
            required: Credits needed (default 1)
            credits_needed: Alias for required (for compatibility)
            credits_remaining: Alias for current (for compatibility)
            message: Custom error message
            upgrade_url: URL for upgrade page
        """
        ...
        self.credits_needed = required
        self.credits_remaining = current
        self.upgrade_url = upgrade_url or "/pricing"

        if message:
            self.message = message
        else:
            self.message = f"Insufficient credits: {current}/{required}"

        super().__init__(self.message, "INSUFFICIENT_CREDITS", 403)
...
def handle_exception(error: Exception) -> tuple:
    ...
    if isinstance(error, VideoAIException):
        ...
        # Add upgrade URL for tier/credit errors
        if isinstance(error, (TierLimitExceeded, InsufficientCreditsError)):
            response["error"]["upgrade_url"] = getattr(error, "upgrade_url", "/pricing")
            if hasattr(error, "current"):
                response["error"]["current"] = error.current
            if hasattr(error, "limit"):
                response["error"]["limit"] = error.limit
            if hasattr(error, "credits_needed"):
                response["error"]["credits_needed"] = error.credits_needed
